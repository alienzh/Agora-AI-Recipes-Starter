...
class AgentViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        chatBackgroundView.onSendImageButtonTapped = { [weak self] in
            // 发送图片按钮点击时的回调
            self?.handleSendImageButtonTapped()
        }
    }
    ...
    private func switchToChatView() {
        ...
        chatBackgroundView.resetInputTextField()
    }
    ...
    // MARK: - Send Image Button Callback
    func handleSendImageButtonTapped() {
        let content = chatBackgroundView.inputTextField.text?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !content.isEmpty else { return }
        sendImage(imageUrl: content)
    }
...
extension AgentViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let messageItem = transcripts[indexPath.row]
        switch messageItem {
...
extension AgentViewController: ConversationalAIAPIEventHandler {
        ...
        if let messageData = error.message.data(using: .utf8) {
            do {
                ...
                if !errorResponse.success {
                    ...
                    print("<<< [ImageUploadError] Image upload failed: \(errorMessage) (code: \(errorCode))")
                    
                    // Update UI to show error state
                    DispatchQueue.main.async { [weak self] in
                        guard let self = self else { return }
                        if let index = self.transcripts.firstIndex(where: {
                            if case .image(let imageItem) = $0 {
                                return imageItem.uuid == errorResponse.uuid
                            }
                            return false
                        }) {
                            if case .image(var imageItem) = self.transcripts[index] {
                                imageItem.status = .failed
                                self.transcripts[index] = .image(imageItem)
                                self.chatBackgroundView.tableView.reloadRows(at: [IndexPath(row: index, section: 0)], with: .none)
                            }
                        }
                    }
                }
        ...
        if messageReceipt.moduleType == .context {
            ...
            if messageReceipt.messageType == .image {
                do {
                    ...
                    print("<<<<<onMessageReceiptUpdated: uuid: \(uuid)")
    ...
    func onTranscriptUpdated(agentUserId: String, transcript: Transcript) {
        DispatchQueue.main.async { [weak self] in
            ...
            if let index = self.transcripts.firstIndex(where: {
                if case .text(let existingTranscript) = $0 {
                    return existingTranscript.turnId == transcript.turnId &&
                           existingTranscript.type.rawValue == transcript.type.rawValue &&
                           existingTranscript.userId == transcript.userId
                }
                return false
            }) {
                self.transcripts[index] = .text(transcript)
            } else {
                self.transcripts.append(.text(transcript))
            }
...
enum ChatMessageItem {
    case text(Transcript)
    case image(ImageMessageItem)
}

struct ImageMessageItem {
    let uuid: String
    let url: String
    var status: ImageMessageStatus
    let timestamp: Date
    
    init(uuid: String, url: String, status: ImageMessageStatus = .sending) {
        self.uuid = uuid
        self.url = url
        self.status = status
        self.timestamp = Date()
