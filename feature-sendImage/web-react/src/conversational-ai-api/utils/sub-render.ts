...
export class CovSubRenderController {
  ...
  protected handleTextMessage(uid: string, message: IUserTranscription) {
    ...
    const user_id = message.user_id || '0'

    const targetChatHistoryItem = this.chatHistory.find(
      (item) => {
        const turnIdMatch = item.turn_id === turn_id
        const objectMatch = item.metadata?.object === messageObject
        const itemUserId = item.metadata?.user_id || item.uid
        const userIdMatch = itemUserId === user_id
        
        if (turnIdMatch && !objectMatch) {
          this.callMessagePrint(
            ELoggerType.debug,
            `[Text Mode] Found item with same turn_id but different object:`,
            `existing: ${item.metadata?.object}, new: ${messageObject}`,
            `existing userId: ${itemUserId}, new userId: ${user_id}`
          )
        }
        
        return turnIdMatch && objectMatch && userIdMatch
      }
    )
    if (!targetChatHistoryItem) {
      ...
      const finalUid = user_id || (message.stream_id
  ...
  private _handleTranscriptChunk() {
    ...
    const targetChatHistoryItem = this.chatHistory.find(
      (item) =>
        ...
        item.metadata?.object === currentTranscript.object
    )
    if (!targetChatHistoryItem) {
      this.callMessagePrint(
        ELoggerType.debug,
  ...
  protected handleMessageInfo(uid: string, message: Record<string, unknown>) {
    try {
      ...
      if (!messageStr || !messageObj || !moduleType || typeof turnId !== 'number') {
        this.callMessagePrint(
          ELoggerType.error,
          'handleMessageInfo',
      ...
      const messageType =
        messageObj?.resource_type === 'picture'
          ? EChatMessageType.IMAGE
          : EChatMessageType.UNKNOWN
