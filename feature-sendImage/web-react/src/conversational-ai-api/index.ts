...
export class ConversationalAIAPI extends EventHelper<IConversationalAIAPIEventHandlers> {
  ...
  public async sendText(agentUserId: string, message: IChatMessageText) {
    ...
    // 将 priority 转换为服务器端期望的格式（大写，不带 ED）
    const priority = message.priority ?? EChatMessagePriority.INTERRUPTED
    let priorityValue: string
    switch (priority) {
      case EChatMessagePriority.INTERRUPTED:
        priorityValue = 'INTERRUPT'
        break
      case EChatMessagePriority.APPEND:
        priorityValue = 'APPEND'
        break
      case EChatMessagePriority.IGNORE:
        priorityValue = 'IGNORE'
        break
      default:
        priorityValue = 'INTERRUPT'
    }
    ...
    try {
      ...
      // 输出实际发送的 payload，用于调试
      console.warn('📤 [RTM_PAYLOAD] ==========================================')
      console.warn('📤 [RTM_PAYLOAD] Actual payload sent to RTM:', payloadStr)
      console.warn('📤 [RTM_PAYLOAD] Payload object:', JSON.stringify(payload, null, 2))
      console.warn('📤 [RTM_PAYLOAD] Options:', JSON.stringify(options, null, 2))
      console.warn('📤 [RTM_PAYLOAD] ==========================================')
