...
命令行脚本，用于启动和停止 Agora 对话式 AI Agent（视觉版本）
...
class AgoraStarterServer:
    """
    Agora Agent Starter Server 实现类（视觉版本）
    用于管理 Agora 对话式 AI Agent 的启动、停止和 Token 生成
    支持视觉（Vision）功能，Agent 可以通过视频获取摄像头画面数据
    """
...
def cmd_start_agent(config: Dict[str, str], dotenv_available: bool = True):
    ...
    try:
        ...
        # 启动 Agent（视觉模式）
        print(f"[INFO] 正在启动 Agent（视觉模式）(app_id={app_id}, channel={channel_name})...")
        print(f"[INFO] Agent RTC UID: {agent_rtc_uid}")
        print(f"[INFO] LLM 输入模态: text, image")
        ...
        print(f"[INFO] Agent RTC UID: {agent_rtc_uid}")
        print(f"\n💡 现在可以打开应用，加入频道 {channel_name} 来体验对话式 AI（视觉）")
    ...
    try:
        ...
        if not agent_id:
            ...
            if not agent_id:
                ...
                print("[ERROR] 使用方式: python agent_start_vision.py stop --agent-id <agent_id>", file=sys.stderr)
                return 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Agora Agent Starter Script (Vision) - 启动和停止 Agora 对话式 AI Agent（视觉版本）'
    )
    subparsers = parser.add_subparsers(dest='command', help='可用命令')
    
    # start 命令
    start_parser = subparsers.add_parser('start', help='启动 Agent（视觉模式，默认启用）')
