...
Command-line script for starting and stopping Agora Conversational AI Agent (Vision version)
...
class AgoraStarterServer:
    """
    Agora Agent Starter Server implementation class (Vision version)
    Used to manage Agora Conversational AI Agent startup, shutdown, and Token generation
    Supports Vision functionality, Agent can get camera video data through video stream
    """
    ...
    def _build_json_payload(
        ...
        """
        Build JSON request body for starting Agent (Vision version)
    ...
    def _execute_join_request(
        ...
        """
        Execute HTTP request to start Agent (Vision version)
...
def cmd_start_agent(config: Dict[str, str], dotenv_available: bool = True):
    """
    Command-line function to start Agent (Vision version)
    ...
    try:
        ...
        print(f"[INFO] Starting Agent (Vision mode) (app_id={app_id}, channel={channel_name})...")
        print(f"[INFO] Agent RTC UID: {agent_rtc_uid}")
        print(f"[INFO] LLM input modalities: text, image")
        ...
        print(f"[INFO] Agent RTC UID: {agent_rtc_uid}")
        print(f"\n💡 You can now open the app and join channel {channel_name} to experience Conversational AI (Vision)")
    ...
    try:
        ...
        if not agent_id:
            ...
            if not agent_id:
                ...
                print("[ERROR] Usage: python agent_start_vision.py stop --agent-id <agent_id>", file=sys.stderr)
                return 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Agora Agent Starter Script (Vision) - Start and stop Agora Conversational AI Agent (Vision version)'
    )
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    
    # start command
    start_parser = subparsers.add_parser('start', help='Start Agent (Vision mode, enabled by default)')
