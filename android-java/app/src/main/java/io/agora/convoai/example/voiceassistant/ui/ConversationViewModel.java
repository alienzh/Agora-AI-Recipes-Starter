...
import io.agora.convoai.example.voiceassistant.rtc.RtmManager.IRtmManagerListener;
...
public class ConversationViewModel extends ViewModel {
    ...
    public static class ConversationUiState {
        ...
        public boolean isTranscriptEnabled = false;
        ...
        public ConversationUiState copy() {
            ...
            newState.isTranscriptEnabled = this.isTranscriptEnabled;
    ...
    private void checkConnectionComplete() {
        if (rtcJoined && rtmLoggedIn) {
            ...
            if (currentState != null) {
                ...
                newState.statusMessage = "RTC and RTM connected successfully";
                _uiState.setValue(newState);
            }
        }
    }
    ...
    public void joinChannelAndLogin(String channelName) {
        try {
            ...
            if (unifiedToken == null || unifiedToken.isEmpty()) {
                ...
                // Use existing token
                RtcManager.joinChannel(unifiedToken, channelName, userId);

                // Login RTM with the same unified token
                RtmManager.login(unifiedToken, errorInfo -> {
                    mainHandler.post(() -> {
                        if (errorInfo == null) {
                            rtmLoggedIn = true;
                            ConversationUiState state = _uiState.getValue();
                            if (state != null) {
                                ConversationUiState newState = state.copy();
                                newState.statusMessage = "RTM logged in successfully";
                                _uiState.setValue(newState);
                            }
                            conversationalAIAPI.subscribeMessage(channelName, (io.agora.convoai.convoaiApi.ConversationalAIAPIError error) -> {
                                if (error != null) {
                            ...
                            if (state != null) {
                                ConversationUiState newState = state.copy();
                                newState.connectionState = ConnectionState.Error;
                                newState.statusMessage = "RTM login failed: " + (errorInfo != null ? errorInfo.getMessage() : "Unknown error");
                                _uiState.setValue(newState);
                            }
                            Log.e(TAG, "RTM login failed: " + (errorInfo != null ? errorInfo.getMessage() : "Unknown error"));
                        }
    ...
    public void toggleTranscript() {
        ConversationUiState currentState = _uiState.getValue();
        if (currentState != null) {
            boolean newTranscriptState = !currentState.isTranscriptEnabled;
            ConversationUiState newState = currentState.copy();
            newState.isTranscriptEnabled = newTranscriptState;
            newState.statusMessage = newTranscriptState ? "Transcript enabled" : "Transcript disabled";
            _uiState.setValue(newState);
