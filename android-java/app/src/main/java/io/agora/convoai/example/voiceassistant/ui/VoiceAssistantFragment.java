...
import io.agora.convoai.example.voiceassistant.java.databinding.FragmentVoiceAssistantBinding;
import io.agora.convoai.example.voiceassistant.java.databinding.ItemTranscriptAgentBinding;
import io.agora.convoai.example.voiceassistant.java.databinding.ItemTranscriptUserBinding;
...
/**
 * VoiceAssistantFragment - Main interface for voice conversation with AI agent
 */
public class VoiceAssistantFragment extends BaseFragment<FragmentVoiceAssistantBinding> {
    ...
    @Override
    protected FragmentVoiceAssistantBinding getViewBinding(LayoutInflater inflater, ViewGroup container) {
    ...
    public void initView() {
        FragmentVoiceAssistantBinding binding = getBinding();
        if (binding == null) {
            return;
        }
        ...
        binding.btnTranscript.setOnClickListener(v -> viewModel.toggleTranscript());
     ...
     * Setup TextView for scrollable status with max height
     */
    private void setupScrollableStatus() {
        FragmentVoiceAssistantBinding binding = getBinding();
        if (binding == null) {
            return;
        }
        ...
        float textSize = binding.tvStatus.getTextSize();
        int maxHeight = (int) (textSize * 5);
        binding.tvStatus.setMaxHeight(maxHeight);

        // Enable scrolling
        binding.tvStatus.setVerticalScrollBarEnabled(true);
    ...
    private void observeUiState() {
        viewModel.uiState.observe(getViewLifecycleOwner(), state -> {
            ...
            binding.cardTranscript.setVisibility(state.isTranscriptEnabled ? View.VISIBLE : View.GONE);

            // Show agent status indicator when transcript is hidden
            binding.agentSpeakingIndicator.setVisibility(state.isTranscriptEnabled ? View.GONE : View.VISIBLE);
            ...
            binding.btnTranscript.setImageResource(
                state.isTranscriptEnabled ? R.drawable.ic_subtitles : R.drawable.ic_subtitles_off
            );
        });
        ...
        viewModel.agentState.observe(getViewLifecycleOwner(), agentState -> {
            ...
            if (agentState != null) {
                ...
                if (agentState == AgentState.SPEAKING) {
                    binding.agentSpeakingIndicator.startAnimation();
                } else {
                    binding.agentSpeakingIndicator.stopAnimation();
                }
            } else {
    ...
    private void updateStatusMessage(ConversationViewModel.ConversationUiState state) {
        ...
        if (state.statusMessage != null) {
            String message = state.statusMessage.toLowerCase();
            if (message.contains("muted") || 
                message.contains("unmuted") || 
    ...
    private void updateStatusDisplay() {
        ...
        binding.tvStatus.post(() -> {
            if (binding.tvStatus.getLayout() != null) {
                int scrollAmount = binding.tvStatus.getLayout().getLineTop(binding.tvStatus.getLineCount());
                if (scrollAmount > binding.tvStatus.getHeight()) {
                    binding.tvStatus.scrollTo(0, scrollAmount - binding.tvStatus.getHeight());
                }
            }
        });
    }
