# agent-starter-convoai-android-java
...
- **开发环境**：
  ...
  - JDK 11 或更高版本
  - Gradle 8.13.0
  - Kotlin 2.0.21（用于 convoaiApi 模块）
...
4. **配置 Agent 启动方式**：
   ...
   有两种方式启动 Agent，在 `AgentStarter.java` 中直接切换：
   ...
   **方式一：本地 HTTP 服务器模式**（推荐用于开发测试）
   ...
   2. 在 `AgentStarter.java` 中配置本地服务器 URL：
   ```java
   ...
   //    private static final String AGORA_API_BASE_URL = "https://api.sd-rtn.com/cn/api/conversational-ai-agent/v2/projects";
       private static final String AGORA_API_BASE_URL = "http://10.0.2.2:8080";  // Android Emulator
   //    private static final String AGORA_API_BASE_URL = "http://192.168.1.100:8080";  // Physical device (replace with your computer IP)
   }
   ```
   ...
   **方式二：直接调用 Agora API 模式**（推荐用于生产环境）
   ...
   在 `AgentStarter.java` 中配置：
   ```java
   public class AgentStarter {
       // Switch between local server and Agora API by commenting/uncommenting the lines below
       private static final String AGORA_API_BASE_URL = "https://api.sd-rtn.com/cn/api/conversational-ai-agent/v2/projects";
   //    private static final String AGORA_API_BASE_URL = "http://10.0.2.2:8080";  // Local server
   }
   ```
   
   **注意**：URL 切换在 `AgentStarter.java` 中完成，不再使用 `env.properties` 中的 `agentServerUrl` 配置。
...
1. **启动 Python HTTP 服务器**（如果使用 HTTP 服务器模式）：
   ...
   服务器启动后，Android 应用会自动通过 `AgentStarter.java` 中配置的地址调用服务器来启动 Agent。
...
│   │   │   │       │   ├── AgentConfigFragment.java    # Agent 配置界面
│   │   │   │       │   ├── VoiceAssistantFragment.java # 语音助手界面
│   │   │   │       │   ├── ConversationViewModel.java  # 对话 ViewModel
│   │   │   │       │   └── common/               # 通用 UI 组件
│   │   │   │       ├── rtc/
│   │   │   │       │   ├── RtcManager.java        # RTC 管理器
│   │   │   │       │   └── RtmManager.java       # RTM 管理器
│   │   │   │       ├── tools/
│   │   │   │       │   ├── AgentStarter.java     # Agent 启动器
│   │   │   │       │   ├── TokenGenerator.java    # Token 生成器
│   │   │   │       │   ├── PermissionHelp.java   # 权限帮助类
│   │   │   │       │   └── Base64Encoding.java    # Base64 编码工具
│   │   │   │       └── net/
│   │   │   │           ├── SecureOkHttpClient.java # 安全 HTTP 客户端
│   │   │   │           └── HttpLogger.java        # HTTP 日志记录器
│   │   │   └── res/                              # 资源文件
...
- [Kotlin 版本](../android-kotlin/README.md)
- 其他 Agora 示例项目
