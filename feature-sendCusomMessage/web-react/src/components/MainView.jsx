...
import { EConversationalAIAPIEvents, EAgentState, EChatMessageType, EChatMessagePriority, EMessageType } from '../conversational-ai-api/type'
...
function MainView({ addLog, clearLogs }) {
  ...
  const setupConvoAIEvents = () => {
    ...
    convoAIAPI.on(EConversationalAIAPIEvents.TRANSCRIPT_UPDATED, (chatHistory) => {
      ...
      // 注释掉详细日志，减少日志输出
      // console.log('[ConvoAI] Raw chat history:', chatHistory.map(item => ({
      ...
      // 注释掉详细日志，减少日志输出
      // console.log('[ConvoAI] Processed transcripts:', newTranscripts.map(t => ({
      //   id: t.id,
      //   type: t.type,
    ...
    if (channelName.trim() && !isConnecting) {
      ...
      try {
        ...
        const convoAIAPI = ConversationalAIAPI.init({
          ...
          enableLog: false, // 关闭内部日志，减少输出（语音播报时会频繁触发）
  ...
  const sendMessage = async () => {
    ...
    // 发送消息的关键日志 - 使用 console.warn 使其在日志中更明显
    console.warn('🚀 [SEND_MESSAGE] ==========================================')
    console.warn('🚀 [SEND_MESSAGE] Sending text message:', text)
    console.warn('🚀 [SEND_MESSAGE] Message object:', JSON.stringify(message, null, 2))
    console.warn('🚀 [SEND_MESSAGE] Priority:', message.priority, '(should be "interrupted")')
    console.warn('🚀 [SEND_MESSAGE] ==========================================')
    ...
    try {
      ...
      setInputText('') // 清空输入框
      safeAddLog('消息发送成功', 'success')
    } catch (error) {
      console.error('[Chat] Failed to send message:', error)
      const errorMessage = error?.message || error?.toString() || '未知错误'
      safeAddLog(`消息发送失败: ${errorMessage}`, 'error')
    } finally {
      setIsSending(false)
    }
  }
...
// 输入框容器
function InputContainer({ inputText, onInputChange, onSend, onKeyPress, isSending }) {
  return (
