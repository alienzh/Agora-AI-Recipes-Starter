...
Command-line script for starting and stopping Agora Conversational AI Agent (Keyword Interrupt version)
...
class AgoraStarterServer:
    """
    Agora Agent Starter Server implementation class (Keyword Interrupt version)
    Used to manage Agora Conversational AI Agent startup, shutdown, and Token generation
    Supports keyword interrupt functionality
    """
    ...
    def _build_json_payload(
        ...
        """
        Build JSON request body for starting Agent (Keyword Interrupt version)
        ...
        Args:
            ...
            interrupt_keywords: List of interrupt keywords
    ...
    def _execute_join_request(
        ...
        """
        Execute HTTP request to start Agent (Keyword Interrupt version)
  ...
  -d '{payload_json}'"""
    ...
    def start_agent(
        ...
        """
        Start an Agent (Keyword Interrupt version)
        ...
        Args:
            ...
            interrupt_keywords: List of interrupt keywords (required)
        ...
        # Validate interrupt keywords list
        if not interrupt_keywords or len(interrupt_keywords) == 0:
            raise ValueError("interrupt_keywords cannot be empty, must provide at least one interrupt keyword")
...
def parse_interrupt_keywords(keywords_str: str) -> List[str]:
    """
    Parse interrupt keywords string
    Supports comma-separated string, e.g.: "stop,pause,end"
    
    Args:
        keywords_str: Keywords string (comma-separated)
...
def validate_config(config: Dict[str, str]):
    ...
    if missing:
        error_msg = f"Missing configuration: {', '.join(missing)} must be set in .env.local file or environment variables"
        raise ValueError(error_msg)


def cmd_start_agent(config: Dict[str, str]):
    """
    Command-line function to start Agent (Keyword Interrupt version)
    Keyword interrupt is enabled by default
    ...
    try:
        ...
        # Parse interrupt keywords
        interrupt_keywords = parse_interrupt_keywords(interrupt_keywords_str)
        if not interrupt_keywords:
            raise ValueError("AGORA_INTERRUPT_KEYWORDS cannot be empty, must provide at least one interrupt keyword")
        
        # Validate keyword count (max 128)
        if len(interrupt_keywords) > 128:
            raise ValueError(f"Interrupt keywords count cannot exceed 128, currently have {len(interrupt_keywords)}")
        ...
        # Start Agent (keyword interrupt mode)
        print(f"[INFO] Starting Agent (keyword interrupt mode) (app_id={app_id}, channel={channel_name})...")
        print(f"[INFO] Agent RTC UID: {agent_rtc_uid}")
        print(f"[INFO] Interrupt keywords: {', '.join(interrupt_keywords)}")
        ...
        print(f"[INFO] Agent RTC UID: {agent_rtc_uid}")
        print(f"[INFO] Interrupt mode: keywords")
        print(f"[INFO] Interrupt keywords: {', '.join(interrupt_keywords)}")
        print(f"\n💡 You can now open the app and join channel {channel_name} to experience Conversational AI (keyword interrupt)")
    ...
    try:
        ...
        if not agent_id:
            ...
            if not agent_id:
                ...
                print("[ERROR] Usage: python agent_interrupt_keyword.py stop --agent-id <agent_id>", file=sys.stderr)
                return 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Agora Agent Starter Script (Keyword Interrupt) - Start and stop Agora Conversational AI Agent (Keyword Interrupt version)'
    )
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    
    # start command
    start_parser = subparsers.add_parser('start', help='Start Agent (keyword interrupt mode, enabled by default)')
