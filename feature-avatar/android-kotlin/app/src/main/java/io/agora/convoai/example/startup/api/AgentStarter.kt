...
object AgentStarter {
    ...
    /**
     * Avatar configuration for digital human feature
     */
    data class AvatarConfig(
        val avatarUid: String,
        val avatarToken: String
    )
     ...
     * @param userUid User RTC UID (required when avatarConfig is provided)
     * @param avatarConfig Avatar configuration (optional, for digital human feature)
    ...
    suspend fun startAgentAsync(
        ...
        try {
            ...
            // Avatar mode requires specific user UID, cannot use wildcard ["*"]
            val remoteRtcUids = if (avatarConfig != null && userUid != null) {
                listOf(userUid)
            } else {
                listOf("*")
            }
    ...
    private fun buildJsonPayload(
        ...
        avatarConfig?.let { config ->
            properties["avatar"] = mapOf(
                "params" to mapOf(
                    "agora_token" to config.avatarToken,
                    "agora_uid" to config.avatarUid
                )
            )
        }
