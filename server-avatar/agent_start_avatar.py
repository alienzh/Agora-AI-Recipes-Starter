...
Command-line script for starting and stopping Agora Conversational AI Agent (Avatar version)
...
class AgoraStarterServer:
    """
    Agora Agent Starter Server implementation class (Avatar version)
    Used to manage Agora Conversational AI Agent startup, shutdown, and Token generation
    Supports Avatar functionality
    """
    ...
    def _build_json_payload(
        ...
        """
        Build JSON request body for starting Agent (Avatar version)
        ...
        Args:
            ...
            avatar_rtc_uid: Avatar RTC UID (optional, for Avatar functionality)
            avatar_rtc_token: Avatar RTC Token (optional, for Avatar functionality)
        ...
        # Add Avatar configuration (if avatar_rtc_uid and avatar_rtc_token are provided)
    ...
    def _execute_join_request(
        ...
        """
        Execute HTTP request to start Agent (Avatar version)
        ...
        Args:
            ...
            token: Token string
            remote_rtc_uids: List of remote RTC UIDs
            avatar_rtc_uid: Avatar RTC UID (optional)
            avatar_rtc_token: Avatar RTC Token (optional)
...
def cmd_start_agent(config: Dict[str, str]):
    """
    Command-line function to start Agent (Avatar version)
    ...
    try:
        ...
        # Generate Avatar Token (Avatar is enabled by default)
        print(f"[INFO] Generating Avatar Token (app_id={app_id}, channel={channel_name}, uid={avatar_rtc_uid})...")
        avatar_token = token_server.generate_token(
            channel_name=channel_name,
            uid=avatar_rtc_uid,
            token_types=token_types
        )
        print(f"[INFO] Avatar Token generated successfully")
        ...
        # Note: When Avatar is enabled, cannot use ["*"] to subscribe to all users, must specify specific UID
        remote_rtc_uids = [current_rtc_uid]
        
        print(f"[INFO] Starting Agent (Avatar mode) (app_id={app_id}, channel={channel_name})...")
        print(f"[INFO] Agent RTC UID: {agent_rtc_uid}")
        print(f"[INFO] Avatar RTC UID: {avatar_rtc_uid}")
        print(f"[INFO] Current RTC UID (for client use): {current_rtc_uid}")
        ...
        print(f"[INFO] Agent RTC UID: {agent_rtc_uid}")
        print(f"[INFO] Avatar RTC UID: {avatar_rtc_uid}")
        print(f"[INFO] Current RTC UID (client should use this UID to join channel): {current_rtc_uid}")
        print(f"\n💡 You can now open the app and use UID {current_rtc_uid} to join channel {channel_name} to experience Conversational AI (Avatar)")
    ...
    try:
        ...
        if not agent_id:
            ...
            if not agent_id:
                ...
                print("[ERROR] Usage: python agent_start_avatar.py stop --agent-id <agent_id>", file=sys.stderr)
                return 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Agora Agent Starter Script (Avatar) - Start and stop Agora Conversational AI Agent (Avatar version)'
    )
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    
    # start command
    start_parser = subparsers.add_parser('start', help='Start Agent (Avatar mode, enabled by default)')
